...
const studentNav = [
  ...
  { label: 'Opportunities', icon: 'work', path: '/student/opportunities' },
  { label: 'My Applications', icon: 'list_alt', path: '/student/applications' },
];
    ...
    <div className="flex h-screen overflow-hidden bg-[#f8fafc]">
      ...
      >
        <div className="p-5 flex items-center gap-3 border-b border-white/10">
          ...
          {open && (
            <div className="overflow-hidden">
              ...
              <p className="text-[9px] text-white/40 uppercase tracking-[0.2em]">Student Portal</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5 custom-scrollbar">
          {studentNav.map(({ label, icon, path }) => (
            <NavLink
              key={path}
              to={path}
              className={({ isActive }) =>
        ...
        <div className="p-4 border-t border-white/10">
          {open ? (
            ...
            <button
              onClick={() => { logoutUser(); navigate('/login'); }}
              className="w-full flex justify-center py-2.5 text-white/60 hover:text-white"
            >
          ...
          <div className="flex items-center gap-4 ml-6">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-gray-100">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-black text-gray-900 leading-none">{user?.name || user?.username?.split('@')[0] || 'Student'}</p>
                <p className="text-[11px] text-gray-400 mt-0.5">Student</p>
              </div>
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center text-white font-black text-sm ring-2 ring-white shadow-sm">
                {(user?.username || 'S').charAt(0).toUpperCase()}
              </div>
            </div>
          </div>
